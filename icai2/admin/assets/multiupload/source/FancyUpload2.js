...
var FancyUpload2 = new Class({
	...
	onStart: function() {
		...
		document.getElementById('upload_clear').style.display="block";
...
FancyUpload2.File = new Class({
	...
	onComplete: function() {
		...
		var json = new Hash(JSON.decode(this.response.text, true) || {});
		/*if(json.get('error')==null)
		{
			var currentcount =parseFloat(document.getElementById('cuurent_uploaded').innerHTML);
				currentcount=currentcount+1;
			document.getElementById('cuurent_uploaded').innerHTML=currentcount;
				addImage(json.get('name'));
		}
		else
		{
document.getElementById('img_error').innerHTML='<div class="error-message" style="width:484px;margin-top:15px;">'+json.get('error')+'</div>';
