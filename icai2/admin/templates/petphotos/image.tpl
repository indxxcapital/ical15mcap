...
window.addEvent('domready', function() { // wait for the content
	...
	var up = new FancyUpload2($('image-status'), $('upload-list'), { // options object
		...
		url: '{$BASE_URL}index.php?module=ajax&event=multiUpload&filetype=pet-photos&imgwith=137&imgheight=110',
		...
		onFileSuccess: function(file, response) { 
		 ...
		 jQuery('#animalImage > tbody:last').append('<tr id=tr-image_'+(rowCount)+' ><td    align="center"><img src="'+ json.get('imageUrl') +'"><input id=imgId_'+(rowCount)+' type="hidden" name="animalImages[]" value="'+ json.get('name')+ '" ></td><td  align="center"><input  checked="checked" type="radio" name="baseImage" value="'+ json.get('name')+ '"></td><td  align="center"><img class="delete" src="assets/images/close.png"  onclick="deleteImageRow('+rowCount+')" /></td></tr>');  
	...
	function deleteImageDatabaseRow(rowId, imageId){		
			...
			if(confirm("Are you sure you want to delete this image?")){
				jQuery.post(BASEURL+"index.php?module=ajax&event=deteteAnimalImage", {animal_image_id: ""+imageId+""}, function(data){
																																 jQuery('#tr-image_'+rowId).remove();
