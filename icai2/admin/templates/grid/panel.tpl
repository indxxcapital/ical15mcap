...
<form action="index.php?module={$currentClass}" method="get" name="frm" onSubmit="go('DisplayRecordsGo')">
		  ...
		  {foreach from=$smarty.get item=HiddenVars key=key}
          {if !in_array($key,$hiddenVarsSystem) && !$key|strstr:"check_" && !$key|strstr:"display"}
          	<input type="hidden" name="{$key}" value="{$HiddenVars}">
          {/if}
          {/foreach}
		  {if $panelHiddenVar}
     ...
     {if $gridItemDiabled.add}
		...
		{else}
	    <div class="float-right"> <a href="index.php?module={$currentClass}&event=addNew" class="button"> <span>Add New <img src="assets/images/plus-small.gif" width="12" height="9" alt="New article" /></span> </a>
        {if $currentClass=="ccharacteristics" || $currentClass == 'cbreakdown' || $currentClass =='cconstituents'}
        <a href="index.php?module={$currentClass}&event=upload" class="button"> <span>Upload CSV <img src="assets/images/plus-small.gif" width="12" height="9" alt="Upload" /></span> </a> 
	{/if}</div>
    	{/if}
	 {/if}
	{if $gridData|@count>0} 
...
{foreach from=$tabHeading item=heading key=p}
<th title="{if $heading.tooltip ne '' }{$heading.tooltip}{else}{$heading.title}{/if}" style="cursor:pointer;"  {if $heading.sortby}class="header{if $heading.sortby==$sortby}{if $sortDirection=="up"} headerSortUp{else} headerSortDown{/if}{/if}" onclick="javascript:sort_col('{$heading.sortby}');"{/if}>{$heading.title}</th>
{/foreach}
<th {if $currentClass=='newslettercontent'} width="180px"{else} width="70px"{/if}>Action</th>
</tr>
</thead>
 {if $gridData|@count>0} 
<tbody>
              ...
              <tr class="{$smarty.capture.classornot}">
               {if $gridItemDiabled.status || $gridItemDiabled.delete} <td class="align-center"><input name='check_{$k}' id='check_{$k}' type='checkbox' style='size:10px;border:0px;' value='{$dataList.id}'>
				...
				{foreach from=$tabHeading item=heading key=p}
                	...
                	<td>{if $heading.coloum_type=="image"}
                    <img src="{$SITE_URL}media/{$heading.folder}/thumb/{$dataList[$heading.coloum]}" />
                    {else}{$dataList[$heading.coloum]}{/if}</td>
                ...
                {if $currentClass eq 'urgentpets' ||  $currentClass eq 'urgentpetinfo'}
                <td align="center" style="width:90px;">
                {elseif $currentClass eq 'pettransport'}
                ...
                {/if}
                    ...
                    <a href="index.php?module={$item.module}{if $item.event}&event={$item.event}{/if}{foreach from=$item.id key=subIds item=subId}&{$subIds}={$dataList[$subId]}{/foreach}" class='a2'>{$item.title} {if $item.title}|{/if}
