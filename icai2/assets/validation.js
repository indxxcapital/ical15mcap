	...
	if(blank=="1" && type!="checkbox" && type!="radio" && type!="dob" && type!="onlyif")
	{
		...
		if(checkExtra=="")
		{
				if(isEmpty(checkvalue)) {		
					jQuery("#"+id).addClass('error');
					jQuery("#error_"+id).addClass('validation-advice');
					jQuery("#label_"+id).addClass('show');
					jQuery("#error_"+id).html(message);
					return false;
...
if(start<=end) {
			...
			jQuery("#"+id).addClass('error');
			jQuery("#error_"+id).addClass('validation-advice');
			jQuery("#error_"+id).html('End date time can not be less than Start date time');
			return false;
			   }
			   else
			   {
	...
	else if(type=="checkbox" || type=="radio")
	{
 	if(id=="tc")
	{
		if(jQuery("#"+id+'_1').is(':checked')==false) {	
    		jQuery("#"+id).addClass('error');
			jQuery("#error_"+id).addClass('validation-advice');
			jQuery("#label_"+id).addClass('show');
			jQuery("#error_"+id).html(message);
	...
	else{if(jQuery('input[name='+id+']').is(':checked')==false) {	
    		jQuery("#"+id).addClass('error');
			jQuery("#error_"+id).addClass('validation-advice');
			jQuery("#label_"+id).addClass('show');
			jQuery("#error_"+id).html(message);
...
function ClearOptions(OptionList) { 
  ...
  if(f.billingtoo.checked == true) {
	...
	f.shippingState.value = ajaxNew('ajax','GetStates','billingCountry','shippingState',f.billingCountry.value,f.billingState.value);
	f.shippingCity.value = f.billingCity.value;
	f.shippingMobile.value = f.billingMobile.value;
  ...
  f.shippingName.value = '';
	...
	f.shippingState.value = '';
	f.shippingCity.value = '';
	f.shippingMobile.value ='';
	   }
}