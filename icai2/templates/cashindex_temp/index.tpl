    ...
    <tbody>
        ...
        <tr>
            ...
            <td>{if $point.status==0}<span class="label label-important">Not approved!</span>{else}<span class="badge badge-success">Approved</span>{/if}</td>
             <td>{if $point.db_approve==0}<span class="label label-important">Not Prepared!</span>{else}<span class="badge badge-success">Prepared</span>{/if}</td>
            <td>
                <div class="btn-group">
                    <!--<a class="btn btn-small show-tooltip" title="View" href="#"><i class="icon-zoom-in"></i></a>-->
                  <a class="btn btn-small show-tooltip" title="Edit" href="index.php?module=cashindextemp&event=view&id={$point.id}"><i class="icon-zoom-in"></i></a>
                      {if $sessData.User.type==1}  <a class="btn btn-small btn-danger show-tooltip" title="Delete" href="#" id="a1" onclick="confirmdelete({$point.id})"><i class="icon-trash"></i></a>
                    {/if}
                </div>
            </td>
        </tr>
