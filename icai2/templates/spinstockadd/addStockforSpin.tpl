...
<!--            <td><a data-original-title="{$point.mnemonic}" data-content="{$sessData.variable[$point.mnemonic]}" data-placement="top" data-trigger="hover" class="show-popover" href="#">{$point.mnemonic}</a></td>-->
        ...
        <tr>
             <td><input type="checkbox" id="checkboxtempid"  name="checkboxtempid[]" value="{$point.indxx_id}_{$smarty.get.id}" {if in_array($point.indxx_id, $selectlive)} selected="selected"{/if}/> </td>
            <td>{$point.name}</td>
            <td>{$point.code}</td>
