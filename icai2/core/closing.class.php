...
$type="close";
		...
		$final_array=array();
			...
			while(false != ($row = mysql_fetch_assoc($indxxs)))
		{
			$MarketCapinsertArray=array();
			
		$date=	$this->db->getResult("select date from tbl_indxx_value where indxx_id='".$row['id']."' order by id desc",true,2);
			//$this->pr($date,true);
		$marketcap=array();
		
		if(!empty($date[1]))
		{
			//echo "select marketcap,isin from tbl_marketcap_close where indxx_id='".$row['id']."' and date='".$date['1']['date']."' order by id desc";
			$oldMktCap=$this->db->getResult("select marketcap,isin from tbl_marketcap_close where indxx_id='".$row['id']."' and date='".$date['1']['date']."' order by id desc",true);
		if(!empty($oldMktCap))
		{
		foreach($oldMktCap as $oldData){
		$marketcap[$oldData['isin']]['marketcap']=$oldData['marketcap'];
		}	
		
		}
		
		//echo "select share,price,isin from tbl_weights where indxx_id='".$row['id']."' and date='".$date['1']['date']."' order by id desc";
		$oldweight=$this->db->getResult("select share,price,isin from tbl_weights where indxx_id='".$row['id']."' and date='".$date['1']['date']."' order by id desc",true);
		if(!empty($oldweight))
		{
		foreach($oldweight as $oldData){
		$marketcap[$oldData['isin']]['share']=$oldData['share'];
		$marketcap[$oldData['isin']]['price']=$oldData['price'];
		}	
	...
	//	$this->pr($marketcap);
		...
		$query = "SELECT  it.isin, 
							 fp.price as calcprice, sh.share as calcshare 
							FROM `tbl_indxx_ticker` it left join tbl_final_price fp on fp.isin=it.isin 
							left join tbl_share sh on sh.isin=it.isin where it.indxx_id='" . $row['id'] . "' 
							and fp.indxx_id='" . $row['id'] . "' and sh.indxx_id='" . $row['id'] . "' and fp.date='" . $datevalue . "'";			
			...
			$indxxpricesres=	mysql_query($query);
					...
					while(false != ($security = mysql_fetch_assoc($indxxpricesres)))
		{
			//$this->pr($security);
			 //echo $security['isin']."=>".$marketcap[$security['isin']]['marketcap'];
			//echo "=>";
			 //echo $security['calcshare']*$security['calcprice']."=>".$marketcap[$security['isin']]['price']*$marketcap[$security['isin']]['share'];
			 //echo "=>";
			 $a=((($security['calcshare']*$security['calcprice'])/($marketcap[$security['isin']]['price']*$marketcap[$security['isin']]['share']))-1);
			//echo "=>";
			  $newMktCap=$marketcap[$security['isin']]['marketcap']*(1+$a);
			//echo "<br>";
		
			$MarketCapinsertArray[]="('". $row['id']."','".$security['isin']."','".$datevalue."','".$newMktCap."')";
	
		}
			if(!empty($MarketCapinsertArray)){
		//echo "insert into tbl_marketcap_close (indxx_id,isin,date,marketcap) values ".implode(",",$MarketCapinsertArray).";";
		$this->db->query("insert into tbl_marketcap_close (indxx_id,isin,date,marketcap) values ".implode(",",$MarketCapinsertArray).";");
		}
			//exit;
...
$type="close";
		...
		$date=	$this->db->getResult("select date from tbl_indxx_value_temp where indxx_id='".$row['id']."' order by id desc",true,2);
			//$this->pr($date,true);
		$marketcap=array();
		
		if(!empty($date[1]))
		{
			$oldMktCap=$this->db->getResult("select marketcap,isin from tbl_marketcap_close_temp where indxx_id='".$row['id']."' and date='".$date['1']['date']."' order by id desc",true);
		if(!empty($oldMktCap))
		{
		foreach($oldMktCap as $oldData){
		$marketcap[$oldData['isin']]=$oldData['marketcap'];
		}	
		...
		$this->db->query("insert into tbl_marketcap_close_temp (indxx_id,isin,date,marketcap) values ".implode(",",$MarketCapinsertArray).";");
		}
			//exit;
...
$headers .= 'From: Indexing <indexing@indxx.com>' . "\r\n"."CC: indexing@indxx.com". "\r\n";
		...
		function checkpvchange()
	{
		...
		$url="../multicurrency2/db_backup.php?date=" .date. "&log_file=" . basename(log_file);
/*echo '<script>document.location.href="http://97.74.65.118/icai2/publishcsixls.php";</script>';
*/		//$this->Redirect("index.php?module=calcftpclose","","");		*/
