...
$this->addJs('js/flaty.js');
	...
	function index()
	{
	$this->smarty->assign('pagetitle','short and leveraged Index');
	$this->smarty->assign('bredcrumssubtitle','short and leveraged Index');	
	
	$this->_baseTemplate="inner-template";
	$this->_bodyTemplate="sl/index";
			$this->_title='short and leveraged Index';
	$indxxdata=$this->db->getResult("select * from tbl_indxx_sl",true);
	...
	$_SESSION['CSI']=$_POST;
			...
			$this->Redirect("index.php?module=sl&event=addAdjfactor","Please Select Index and Submit Fraction !!!","success");	
		}
...
//	$this->pr($_SESSION,true);
	...
	$this->_baseTemplate="inner-template";
			$this->_bodyTemplate="sl/addnext";
			$this->_title="Add short and leveraged Factor";
			$this->_meta_description=$this->siteconfig->default_meta_description;
			$this->_meta_keywords=$this->siteconfig->default_meta_keyword;
	...
	exit;
		...
		$this->db->query("INSERT into tbl_indxx_sl set status='0', name='".mysql_real_escape_string($_SESSION['CSI']['name'])."',code='".mysql_real_escape_string($_SESSION['CSI']['code'])."',bvalue='".mysql_real_escape_string($_SESSION['CSI']['bvalue'])."',zone='".mysql_real_escape_string($_SESSION['CSI']['zone'])."',client_id='".mysql_real_escape_string($_SESSION['CSI']['client_id'])."',user_id='".mysql_real_escape_string($_SESSION['User']['id'])."'");
//	$this->pr($_SESSION['CSI']);
	...
	$this->db->query("INSERT into tbl_sl_adj_factor set cs_indxx_id='".mysql_real_escape_string($newIndxx_id)."',indxx_id='".mysql_real_escape_string($index_id)."',code='".mysql_real_escape_string($indxxdata['code'])."',fraction='".mysql_real_escape_string($_POST['fraction'][$index_key])."'");
//	$this->pr($_SESSION['CSI']);
	//$this->pr($indxxdata);
	}
	
	$this->db->query("INSERT into tbl_indxx_sl_value set indxx_id='".$newIndxx_id."',code='".mysql_real_escape_string($_SESSION['CSI']['code'])."',indxx_value='".mysql_real_escape_string($_SESSION['CSI']['bvalue'])."',date='".mysql_real_escape_string($this->_date)."'");
//	$this->pr($_SESSION['CSI']);
	...
	$emailQueries='select email from tbl_ca_user where status="1" and type="1" ';
		...
		if(!empty($emailsids))	
		{
			...
			$text="New  short and leveraged Index  : ".$_SESSION['CSI']['name']." (".$_SESSION['CSI']['code'].") added by ".$_SESSION['User']['name']."\r\n Please View and Approve It to Run";
...
$headers .= 'From: Indexing <indexing@indxx.com>' . "\r\n"."CC: indexing@indxx.com". "\r\n";
					//echo $emailsids;
						if(mail($emailsids,"New short and leveraged Index ",$msg,$headers))
					{
						echo "Mail Send ";
...
//	$this->pr($_POST,true);
	...
	//$_SESSION['CSI']=$_POST;
			...
			$this->Redirect("index.php?module=sl","Record added successfully!!!","success");	
		}
	...
	function edit()
	{
			$this->_baseTemplate="inner-template";
			$this->_bodyTemplate="sl/addnext";
			$this->_title="Edit short and leveraged Factor";
			$this->_meta_description=$this->siteconfig->default_meta_description;
			$this->_meta_keywords=$this->siteconfig->default_meta_keyword;
		...
		$tickerdata=$this->db->getResult("select tbl_sl_adj_factor.* from tbl_sl_adj_factor where cs_indxx_id='".$_GET['id']."' ",true);
	...
	//echo "updated tbl_indxx_sl set status='0' where  id ='".$_GET['id']."'";
	//exit;
		$this->db->query("update tbl_indxx_sl set status='0' where  id ='".$_GET['id']."'");
//	$this->pr($_SESSION['CSI']);
	//$newIndxx_id=mysql_insert_id();
	$strQuery = "delete from tbl_sl_adj_factor where cs_indxx_id='".$_GET['id']."'";
					$this->db->query($strQuery);
	foreach($_POST['indxx_id'] as $index_key=>$index_id)
	{
	$indxxdata=$this->db->getResult("select code from tbl_indxx where id = '".$index_id."' ",false,1);
	
	$this->db->query("INSERT into tbl_sl_adj_factor set cs_indxx_id='".mysql_real_escape_string($_GET['id'])."',indxx_id='".mysql_real_escape_string($index_id)."',code='".mysql_real_escape_string($indxxdata['code'])."',fraction='".mysql_real_escape_string($_POST['fraction'][$index_key])."'");
//	$this->pr($_SESSION['CSI']);
	//$this->pr($indxxdata);
	}
	...
	$emailQueries='select email from tbl_ca_user where status="1" and type="1" ';
		...
		if(!empty($emailsids))	
		{
			...
			$text="short and leveraged Index  : ".$_SESSION['CSI']['name']." (".$_SESSION['CSI']['code'].") Updated by ".$_SESSION['User']['name']."\r\n Please View and Approve It to Run";
...
//	$this->pr($_POST,true);
	...
	//$_SESSION['CSI']=$_POST;
			...
			$this->Redirect("index.php?module=sl","Record Updated  successfully!!!<br> Please Wait for Admin Approval.","success");	
		}
...
{	   
	...
	protected function delete(){
		 	...
		 	$strQuery1 = "delete from tbl_indxx_sl where id='".$_GET['id']."'";
					$this->db->query($strQuery1);
					
									
					$strQuery = "delete from tbl_sl_adj_factor where tbl_assign_index.cs_indxx_id='".$_GET['id']."'";
					$this->db->query($strQuery);
			
			$this->Redirect("index.php?module=sl","Record deleted successfully!!!","success");
		...
		function deleteindex()
	{
		...
		$this->Redirect("index.php?module=sl","Records deleted successfully!!!","success");
		$this->show();	
	}
	...
	function view()
{
	...
	$this->_baseTemplate="inner-template";
			$this->_bodyTemplate="sl/view";
			$this->_title="View short and leveraged Index";
	
	$indxxdata=$this->db->getResult("select * from tbl_indxx_sl where id = '".$_GET['id']."' ",true);
	

	
	$indxxfactordata=$this->db->getResult("select * from tbl_sl_adj_factor where cs_indxx_id = '".$_GET['id']."' ",true);
...
$indxxfactordata[$key]['name']=$indxxname['name'];
	...
	$indxxdata=$this->db->getResult("select * from tbl_indxx_sl where id = '".$_GET['id']."' ",false,1);
	
	//$this->pr($indxxdata,true);
	
			
			
			$strQuery1 = "update tbl_indxx_sl set status='1' where id='".$_GET['id']."'";
					$this->db->query($strQuery1);
			
				$emailQueries=$this->db->getResult('select email from tbl_ca_user where status="1" and id="'.$indxxdata['user_id'].'" ',false,1);
			
			$text="Your  short and leveraged Index  : ".$indxxdata['name']." (".$indxxdata['code'].") approved  by ".$_SESSION['User']['name']."\r\n ";
...
//exit;
						if(mail($emailQueries['email'],"short and leveraged Index Approved",$msg,$headers))
					{
						echo "Mail Send ";
	...
	$this->Redirect("index.php?module=sl","Records Approved successfully!!!","success");
	}
