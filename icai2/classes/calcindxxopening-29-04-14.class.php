...
if($type="open"){
	  ...
	  file_put_contents('../files2/backup/preopendata'.date("Y-m-d-H-i-s").'.json', json_encode($final_array));
	 // exit;
...
//$this->pr($ca_prices);
	 ...
	 file_put_contents('../files2/backup/postopendata'.date("Y-m-d-H-i-s").'.json', json_encode($final_array));	
	}
	//$this->pr($final_array,true);
	if(!empty($final_array))
	{
