...
class Calcstockaddmarketcaptemp extends Application{
...
//echo "select tbl_indxx_value.* from tbl_indxx_value where indxx_id='".$indxx['indxx_id']."' order by date desc ";
		...
		//	$this->pr($indxx_value,true);
			...
			//$final_array[$indxx['id']][$indxx['indxx_id']]['date']=$indxx_value['date'];
				...
				$marketcap=	$this->db->getResult("select sum(marketcap) as total_marketcap from tbl_marketcap_close_temp where indxx_id='".$indxx['id']."' and  date='".$datevalue."' ",false);	
...
foreach($request_array as $id=>$indxx_array)			
{
	...
	$this->db->query($insertshare);	
		...
		$insertmarketcap='Insert into tbl_marketcap_close_temp set date="'.$indxx_array['index_value']['date'].'", isin="'.$newsecurity['isin'].'", marketcap="'.$newsecurity['value'].'",indxx_id="'.$id.'"'; 
	
	$this->db->query($insertmarketcap);	
	
	$insertweights='Insert into tbl_weights_temp set date="'.$indxx_array['index_value']['date'].'", isin="'.$newsecurity['isin'].'", share="'.$share.'",code="'.$indxx_array['index_value']['code'].'",price="'.$newsecurity['calcprice'].'",weight="'.$TempWeight.'",indxx_id="'.$id.'"'; 
	
		$this->db->query($insertweights);
		
		$insertweightsopen='Insert into tbl_weights_open_temp set date="'.$indxx_array['index_value']['date'].'", isin="'.$newsecurity['isin'].'", share="'.$share.'",code="'.$indxx_array['index_value']['code'].'",price="'.$newsecurity['calcprice'].'",weight="'.$TempWeight.'",indxx_id="'.$id.'"'; 
	...
	$this->db->query($insertweightsopen);	
	  ...
	  $query="SELECT  sum(sh.share*price)  as total_market_cap
							FROM tbl_final_price_temp fp
							left join tbl_share_temp sh on sh.isin=fp.isin where 
...
$this->Redirect("index.php?module=calcstockaddmarketcap&log_file=".basename(log_file)."&date=".date,"","");	
