...
if (!in_array($_FILES['inputfile']['type'], $csv_mimetypes)) {
		$check=false;
			...
			if(!empty($ordered))
				...
				$indxx_id=0;
					$this->db->query("insert into tbl_indxx_temp(name,code,type,cash_adjust,specialcashadd,curr,status,usersignoff,submitted,addtype,client_id,display_currency,ireturn,ica,div_type,currency_hedged,dateStart,calcdate) select name,code,type,cash_adjust,specialcashadd,curr,status,usersignoff,submitted,addtype,client_id,display_currency,ireturn,ica,div_type,currency_hedged,'".$_SESSION['reconstitute'][$live_index_id]['startDate']."','".$_SESSION['reconstitute'][$live_index_id]['calcdate']."' from tbl_indxx where code='".$key."' ");
					$indxx_id=mysql_insert_id();
					$this->db->query("update tbl_indxx_temp set recalc='1' where id='".$indxx_id."'");
				...
				$tickerArray[]="('".$indxx_id."','".$ticker[2]."','".$ticker[3]."','".$ticker[4]."','".$ticker[5]."','".$ticker[6]."','".$ticker[7]."','".$ticker[8]."','".$ticker[9]."','".$ticker[10]."','".$ticker[11]."','".$ticker[12]."','".str_replace("%","",$ticker[14])."')";
				}
