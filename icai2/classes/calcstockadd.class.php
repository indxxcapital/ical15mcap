...
class Calcstockadd extends Application{
...
foreach($request_array as $id=>$indxx_array)			
{
	...
	if(!empty($indxx_array['newsecurity']) && !empty($indxx_array['index_value']))
	{
		...
		if($share){
			...
			$insertTicker='Insert into tbl_indxx_ticker set name="'.$newsecurity['name'].'", isin="'.$newsecurity['isin'].'", ticker="'.$newsecurity['ticker'].'", curr="'.$newsecurity['curr'].'", divcurr="'.$newsecurity['divcurr'].'",countryname="'.$newsecurity['countryname'].'",cusip="'.$newsecurity['cusip'].'",sedol="'.$newsecurity['sedol'].'",indxx_id="'.$id.'", status="1",weight="'.$security['value'].'"'; 
	...
	$insertweights='Insert into tbl_weights set date="'.$indxx_array['index_value']['date'].'", isin="'.$newsecurity['isin'].'", share="'.$share.'",code="'.$indxx_array['index_value']['code'].'",price="'.$newsecurity['calcprice'].'",weight="'.$newsecurity['value'].'",indxx_id="'.$id.'"'; 
	
		$this->db->query($insertweights);
		
		$insertweightsopen='Insert into tbl_weights_open set date="'.$indxx_array['index_value']['date'].'", isin="'.$newsecurity['isin'].'", share="'.$share.'",code="'.$indxx_array['index_value']['code'].'",price="'.$newsecurity['calcprice'].'",weight="'.$newsecurity['value'].'",indxx_id="'.$id.'"'; 
	...
	$this->db->query($insertweightsopen);	
	 ...
	 $query="SELECT  it.id, it.name, it.isin, it.ticker, 
							...
							FROM `tbl_indxx_ticker` it 
							left join tbl_final_price fp on fp.isin=it.isin
							left join tbl_share sh on sh.isin=it.isin where it.indxx_id='" . $id . "' 
