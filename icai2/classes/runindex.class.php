...
class Runindex extends Application 
{
	...
	function index() 
	{
		...
		{	
			...
			{
				 $error=$this->checkformarketCap($_POST ['id'],$_POST ['date']);
				if($error)
				{
					$res = $this->Redirect ($this->siteconfig->base_url . 'index.php?module=caindex&event=viewupcoming&id=' . $_POST ['id'],"MarketCap of Following Securities are not available for date ".$_POST['date']."<br>".$error,"error" );
				}
				else{
...
function checkformarketCap($indxx,$date){
$text='';
$array=array();
$flag=false;
