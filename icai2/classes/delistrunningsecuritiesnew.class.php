...
class Delistrunningsecuritiesnew extends Application{
...
///echo "deepak";
	...
	$indexdata=$this->db->getResult("select tbl_indxx_ticker.id from tbl_indxx_ticker where indxx_id='".$indxx_id."' and isin='".$isin."' ",false);
	return $indexdata['id'];
	...
	function addNew2()
	{
		...
		$cas=$this->db->getResult("select tbl_indxx.name,tbl_indxx.code,tbl_indxx.id from tbl_indxx_ticker
...
{			
			...
			//echo "select * from tbl_delist_runnindex_req  where indxx_id='".$ca_id."' and startdate='".$_SESSION['delistDate']."'";
			//exit;
	$query=$this->db->getResult("select * from tbl_delist_runnindex_req  where indxx_id='".$ca_id."' and startdate='".$_SESSION['delistDate']."'",false);
	if(!empty($query))
	$req_id=$query['id'];
	else{
$this->db->query("insert into tbl_delist_runnindex_req  (indxx_id ,startdate,status,user_id) values ('".$ca_id."','".$_SESSION['delistDate']."','1','".$_SESSION['User']['id']."');");
	$req_id=mysql_insert_id();
	}
	
	
	$query=$this->db->getResult("select * from tbl_delist_runnsecurity  where indxx_id='".$ca_id."' and req_id='".$req_id."' and security_id='".$_SESSION['delistsecurity']."'",false);
	if(empty($query)){
$this->db->query("insert into `tbl_delist_runnsecurity`  (status,indxx_id ,security_id,req_id) values ('1','".$ca_id."','".$_SESSION['delistsecurity']."','".$req_id."');");
}
	
}
	
	
	//$this->db->query("insert into tbl_indxx  (indxx_id ,startdate) values ('".$ca_id."','".$_SESSION['delistDate']."');");
...
$body.='New request for Delist security has been submitted  , <br> Please  <a href="'.$this->siteconfig->base_url.'index.php?module=delistrunningsecurities">Click here </a> to do more.<br>Thanks ';
		...
		$this->Redirect("index.php?module=delistrunningsecuritiesnew","Securities to be delisted added successfully!!! <br> Please Wait for Approval","success");
	}
...
{	   
	...
	function addNew()
	{
		...
		/*	
			...
			$this->Redirect("index.php?module=delistrunningsecuritiesnew&event=addNew2&delistDate=".$_POST['startdate']."&isin=".$_POST['isin'],"Please Select index for Security ".$_POST['isin']." for deletion on ".$_POST['startdate'],"success");	
		}
 ...
 $this->validData[]=array("feild_label" =>"Security",
								 ...
								 "feild_type" =>"radio",
								 "is_required" =>"1",
								 "model"=>$this->getRunningSecurities(),
...
$body.='You have new indxx for Delisting, <br> Please  <a href="'.$this->siteconfig->base_url.'index.php?module=delistrunningsecurities&event=viewupcoming&id='.$_GET['reqid'].'">Click here </a> to check it.<br>Thanks ';
	...
	protected function viewupcoming(){
		...
		$securitytobedelisted=$this->db->getResult("select tbl_indxx_ticker.* from tbl_delist_runnsecurity left join tbl_indxx_ticker on tbl_indxx_ticker.isin=tbl_delist_runnsecurity.security_id  where tbl_delist_runnsecurity.req_id='".$_GET['id']."' and  tbl_delist_runnsecurity.indxx_id='".$viewdata[0]['indxx_id']."' and tbl_indxx_ticker.indxx_id='".$viewdata[0]['indxx_id']."'",true);
...
$body.="Delisting in : ".$indxx['name']."(".$indxx['code'].")";
		...
		{
				$this->Redirect("index.php?module=delistrunningsecuritiesnew","You are not authorized to perofrm this task!","error");
		}
