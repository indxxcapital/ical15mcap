	...
	function addNewRunning()
	{
		...
		if(isset($_POST['submit']))
		{
			...
			//$_SESSION['InsertedData']=$_POST;
					...
					{
		$this->db->query("INSERT into tbl_indxx_temp set addtype='1', status='0',name='".mysql_real_escape_string($_POST['name'])."',code='".mysql_real_escape_string(trim($_POST['code']))."',investmentammount='0',divisor='0',type='".mysql_real_escape_string($_POST['type'])."',zone='".mysql_real_escape_string($_POST['zone'])."',curr='".mysql_real_escape_string($_POST['curr'])."',lastupdated='".date("Y-m-d h:i:s")."',dateStart='".$_POST['dateStart']."',cash_adjust='".$_POST['cash_adjust']."',specialcashadd='".$_POST['cash_adjust1']."',client_id='".$_POST['client_id']."',ica='".$_POST['ica']."',display_currency='1',ireturn='".$_POST['ireturn']."',div_type='".$_POST['div_type']."',currency_hedged='".$_POST['currency_hedged']."'");
		//
		$indexid=mysql_insert_id();
...
//exit;
	...
	//echo 'SELECT tbl_indxx_ticker.name,tbl_indxx_ticker.ticker,tbl_indxx_ticker.isin,tbl_indxx_ticker.sedol,tbl_indxx_ticker.cusip,tbl_indxx_ticker.countryname,tbl_indxx_ticker.curr,tbl_indxx_ticker.divcurr,tbl_weights.weight,tbl_weights.price,tbl_weights.share FROM tbl_indxx_ticker left join tbl_weights on tbl_indxx_ticker.isin=tbl_weights.isin  where tbl_indxx_ticker.indxx_id="'.$_GET['id'].'" and tbl_weights.indxx_id="'.$_GET['id'].'" and tbl_weights.date="'.$lastCloseData['date'].'"';
	//print_r($sequruityData);
	//exit;
	if(empty($sequruityData))
$sequruityData=$this->db->getResult('SELECT tbl_indxx_ticker.name,tbl_indxx_ticker.ticker,tbl_indxx_ticker.isin,tbl_indxx_ticker.sedol,tbl_indxx_ticker.cusip,tbl_indxx_ticker.countryname,tbl_indxx_ticker.curr,tbl_indxx_ticker.divcurr FROM tbl_indxx_ticker  where tbl_indxx_ticker.indxx_id="'.$_GET['id'].'" ',true);
	...
	//	$this->pr($sequruityData,true);
	 ...
	 protected function viewupcoming(){
		...
		//echo "a";
		//echo 'SELECT tbl_indxx_ticker_temp.name,tbl_indxx_ticker_temp.ticker,tbl_indxx_ticker_temp.isin,tbl_indxx_ticker_temp.sedol,tbl_indxx_ticker_temp.cusip,tbl_indxx_ticker_temp.countryname,tbl_indxx_ticker_temp.curr,tbl_indxx_ticker_temp.divcurr,tbl_indxx_ticker_temp.sector,tbl_indxx_ticker_temp.industry,tbl_indxx_ticker_temp.subindustry,tbl_weights_temp.weight,tbl_weights_temp.price,tbl_weights_temp.share FROM tbl_indxx_ticker_temp left join tbl_weights_temp on tbl_indxx_ticker_temp.isin=tbl_weights_temp.isin  where tbl_indxx_ticker_temp.indxx_id="'.$_GET['id'].'" and tbl_weights_temp.date="'.$lastCloseData['date'].'"';
		//exit;
		}
		else
	    {$sequruityData=$this->db->getResult('SELECT * FROM tbl_indxx_ticker_temp where indxx_id="'.$_GET['id'].'"',true);
...
//$this->smarty->assign("uploadsecuritydata",$uploadsecuritydata);
	...
	protected function editfornext(){
		...
		$editdata=$this->db->getResult("select tbl_indxx.* from tbl_indxx  where tbl_indxx.id='".$_GET['id']."'");
		//$this->pr($editdata,true);
		//echo $editdata['code']=trim($editdata['code']);
		//exit;
		$this->smarty->assign("postData",$editdata);
		...
		$_SESSION['indxx_type']=$editdata['type'];
			...
			if(empty($checkdata))
			{
					...
					$this->db->query("INSERT into tbl_indxx_temp set status='0',recalc='1',name='".mysql_real_escape_string($_POST['name'])."',code='".mysql_real_escape_string($_POST['code'])."',cash_adjust='".mysql_real_escape_string($_POST['cash_adjust'])."',specialcashadd='".mysql_real_escape_string($_POST['cash_adjust1'])."',curr='".mysql_real_escape_string($_POST['curr'])."',lastupdated='".date("Y-m-d H:i:s")."',dateStart='".$_POST['dateStart']."',client_id='".mysql_real_escape_string($_POST['client_id'])."',display_currency='1',ireturn='".mysql_real_escape_string($_POST['ireturn'])."',ica='".mysql_real_escape_string($_POST['ica'])."',currency_hedged='".mysql_real_escape_string($_POST['currency_hedged'])."',div_type='".mysql_real_escape_string($_POST['div_type'])."'");
		...
		$this->db->query("delete from tbl_spin_stock_add_securities_temp where indxx_id='".$tempindexid."'");
			...
			else
			{
				...
				$this->db->query("UPDATE tbl_indxx_temp set status='0',recalc='1',name='".mysql_real_escape_string($_POST['name'])."',code='".mysql_real_escape_string($_POST['code'])."',type='".mysql_real_escape_string($_POST['type'])."',curr='".mysql_real_escape_string($_POST['curr'])."',lastupdated='".date("Y-m-d H:i:s")."',dateStart='".$_POST['dateStart']."',client_id='".mysql_real_escape_string($_POST['client_id'])."',display_currency='1',ireturn='".mysql_real_escape_string($_POST['ireturn'])."',ica='".mysql_real_escape_string($_POST['ica'])."',currency_hedged='".mysql_real_escape_string($_POST['currency_hedged'])."',cash_adjust='".mysql_real_escape_string($_POST['cash_adjust'])."',specialcashadd='".mysql_real_escape_string($_POST['cash_adjust1'])."' where id='".$checkdata['id']."'");
...
$headers .= 'From: Indexing <indexing@indxx.com>' . "\r\n"."CC: indexing@indxx.com". "\r\n";
$body='Hi <br>';
$body.='You have new upcoming indxx '.$indxx['name']."(".$indxx['code'].")".' for aupload and Approval , <br> Please visit '.$this->siteconfig->base_url.'index.php?module=caindex&event=viewupcoming&id='.$_SESSION['NewIndxxId'].'  to check it.<br>Thanks ';
//echo $body;
//exit;
...
$body.='You have new upcoming indxx for aupload and Approval , <br> Please visit '.$this->siteconfig->base_url.'index.php?module=caindex&event=viewupcoming&id='.$_GET['id'].'  to check it.<br>Thanks ';
 ...
 $this->show();
	...
	//$this->pr($viewdata,true);
	$lastCloseData=$this->db->getResult('SELECT * FROM tbl_indxx_value where indxx_id="'.$_GET['id'].'" order by id desc',false,1);
	$sequruityData=$this->db->getResult('SELECT tbl_indxx_ticker.name,tbl_indxx_ticker.ticker,tbl_indxx_ticker.isin,tbl_indxx_ticker.sedol,tbl_indxx_ticker.cusip,tbl_indxx_ticker.countryname,tbl_indxx_ticker.curr,tbl_indxx_ticker.divcurr,tbl_weights.weight,tbl_weights.price,tbl_weights.share,tbl_marketcap_close.marketcap FROM tbl_indxx_ticker
	 left join tbl_weights on tbl_indxx_ticker.isin=tbl_weights.isin
	 left join tbl_marketcap_close on tbl_indxx_ticker.isin=tbl_marketcap_close.isin
	 
	 where tbl_indxx_ticker.indxx_id="'.$_GET['id'].'" and tbl_weights.indxx_id="'.$_GET['id'].'" and tbl_weights.date="'.$lastCloseData['date'].'" and tbl_marketcap_close.indxx_id="'.$_GET['id'].'" and tbl_marketcap_close.date="'.$lastCloseData['date'].'"',true);
	//$shareData=$this->db->getResult('SELECT * FROM tbl_share where indxx_id="'.$_GET['id'].'"',true);
	$marketcap=$this->db->getResult('SELECT * FROM tbl_marketcap_close where indxx_id="'.$_GET['id'].'"',true);
	...
	if(!empty($marketcap))
	{
	foreach($marketcap as $marketcap)
	{
		
	$marketcap[$marketcap['isin']]=$marketcap['marketcap'];
	}
	}*/
	...
	$data = array(
        ...
        2 => array ($data[0]['name'],$data[0]['code'], $data[0]['curr']),
        3 => array ('Security Name', 'Ticker', 'ISIN','Cusip','Sedol','Country', 'Weight','Price','Share', 'Currency','Market Cap'),
...
foreach($sequruityData as $data)
		{
			$moredata[$k++]=array($data['name'],$data['ticker'],$data['isin'],$data['cusip'],$data['sedol'],$data['countryname'],$data['weight'],$data['price'],$data['share'],$data['curr'],$data['marketcap']);
...
$xls->generateXML('indxx-data-'.$viewdata[0]['code']."-".$this->_date);		
	...
	$data = array(
        ...
        3 => array ('Security Name', 'Ticker', 'ISIN', 'Weight','Price','Share', 'Currency','Dividend Currency','Sedol','Cusip','Country','Sector','Industry','Sub-Industry'),
...
foreach($sequruityData as $data)
		{
			$moredata[$k++]=array($data['name'],$data['ticker'],$data['isin'],$data['weight'],$data['price'],$shares[$data['isin']],$data['curr'],$data['divcurr'],$data['sedol'],$data['cusip'],$data['countryname'],$data['sector'],$data['industry'],$data['subindustry']);
