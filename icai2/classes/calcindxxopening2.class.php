...
class Calcindxxopening2 extends Application{
	...
	function index()
	{
		...
		$indxxs=$this->db->getResult("select tbl_indxx.* from tbl_indxx  where status='1' and usersignoff='1' and dbusersignoff='1' and submitted='1' limit $page,10",true);
	...
	//$this->pr($indxx);
		...
		//	exit;
			...
			{
			 $ca_query="select identifier,action_id,id,mnemonic,field_id,company_name,ann_date,eff_date,amd_date,currency from tbl_ca cat where  eff_date='".$datevalue2."' and identifier='".$indxxprice['ticker']."'  and status='1' and action_id in (select ca_action_id from tbl_ca_admin_approve where ca_action_id=cat.action_id) and action_id in (select ca_action_id from tbl_ca_client_notified where ca_action_id=cat.action_id) and action_id in (select ca_action_id from tbl_ca_admin_notified where ca_action_id=cat.action_id) and action_id not in (select ca_action_id from tbl_ignore_index where ca_action_id=cat.action_id)";
...
//$this->pr($ca_prices);
					...
					$adjfactorforcash=($closeprices['calcprice']-$ca_prices['ca_price_in_ca_currency'])/$closeprices['calcprice'];
...
$filetext= "file Written for ".$closeIndxx['code']."<br>";
		...
		{
			$this->Redirect("index.php?module=calcindxxopening2&event=index&id=".($page+10),"","");	
		}
