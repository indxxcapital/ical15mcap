...
$myids='';
		...
		//exit;
		/*if($myids)
		$indexdata=$this->db->getResult("select name, code,id from tbl_indxx where 1=1 and ".$myids,true);
		else
		$indexdata=$this->db->getResult("select name, code,id from tbl_indxx where 1=1",true);
		$this->adddate();
		*/
...
function addstock(){
		$this->_baseTemplate="inner-template";
			$this->_bodyTemplate="replacerunning/addstock";
			$this->_title=$this->siteconfig->site_title;
			$this->_meta_description=$this->siteconfig->default_meta_description;
			$this->_meta_keywords=$this->siteconfig->default_meta_keyword;
			$this->smarty->assign('pagetitle','Index');
		...
		$this->Redirect("index.php?module=replacerunningsecurities&event=addNew","Invalid input , Please fill details again","error");
		
		$security=$this->db->getResult("select * from tbl_indxx_ticker where isin='".$_SESSION['add_stock']['isin']."'",false,1);
		...
		//exit;
		if($myids)
		$indexdata=$this->db->getResult("select name, code,id from tbl_indxx where 1=1 and status='1' and  ".$myids,true);
		else
		$indexdata=$this->db->getResult("select name, code,id from tbl_indxx where 1=1  and status='1'",true);
		//$this->adddate();
		$this->smarty->assign("indexdata",$indexdata);
    ...
    $req_id=0;	
	$query=$this->db->getResult("select * from tbl_replace_runnindex_req  where indxx_id='".$indxx_id."' and startdate='".$_SESSION['add_stock']['startdate']."'",false);
	if(!empty($query))
	$req_id=$query['id'];
	else{
$this->db->query("insert into tbl_replace_runnindex_req  (indxx_id ,startdate,status,user_id) values ('".$indxx_id."','".$_SESSION['add_stock']['startdate']."','1','".$_SESSION['User']['id']."')");
	$req_id=mysql_insert_id();
	
	
	
	
/*	$query=$this->db->getResult("select * from  tbl_replace_runnsecurity  where indxx_id='".$indxx_id."' and req_id='".$req_id."' and security_id='".$_SESSION['delistsecurity']."'",false);
	
	if(empty($query)){
$this->db->query("insert into ` tbl_replace_runnsecurity`  (status,indxx_id ,security_id,req_id) values ('1','".$indxx_id."','".$_SESSION['delistsecurity']."','".$req_id."');"); 
}*/
	...
	if($req_id)
	{
		...
		$this->db->query("insert into tbl_runnsecurities_replaced  (name,isin,ticker,curr,divcurr,cusip,sedol,countryname,indxx_id ,status,req_id,value) values ('".$_POST['name']."','".$_POST['isin']."','".$_POST['ticker']."','".$_POST['curr']."','".$_POST['divcurr']."','".$_POST['cusip']."','".$_POST['sedol']."','".$_POST['countryname']."','".$indxx_id."','1','".$req_id."','".$_POST['weight']."')");
		}else{
		//update Query///tbl_runnsecurities_replaced
		$query=$this->db->getResult("update  tbl_runnsecurities_replaced set name='".$_POST['name']."',ticker='".$_POST['ticker']."',curr='".$_POST['curr']."',divcurr='".$_POST['divcurr']."',cusip='".$_POST['cusip']."',sedol='".$_POST['sedol']."',countryname='".$_POST['countryname']."',value='".$_POST['weight']."' where indxx_id='".$indxx_id."' and req_id='".$req_id."' and isin='".$_POST['isin']."'",false);
...
$body='Hi <br>';
$body.='New request for Add Stock running index security has been submitted  , <br> Please  <a href="'.$this->siteconfig->base_url.'index.php?module=replacerunningsecurities">Click here </a> to do more.<br>Thanks ';


		mail($to,"ICAI :Security Delist Request " ,$body,$headers);
		
		$this->Redirect("index.php?module=replacerunningsecurities","Stock for running indexto be  added successfully!!! <br> Please Wait for Approval","success");
		...
		if($_POST['submit'])
		{
			...
			for($i=1;$i<$_POST['totalfields'];$i++)
			{
				if($_POST['name'][$i] && $_POST['isin'][$i] && $_POST['ticker'][$i] && $_POST['divcurr'][$i] && $_POST['curr'][$i])
				{
					...
					//$this->pr($_POST,true);	
					$this->db->query("INSERT into tbl_runnsecurities_replaced set status='0',name='".mysql_real_escape_string($_POST['name'][$i])."',isin='".mysql_real_escape_string($_POST['isin'][$i])."',ticker='".mysql_real_escape_string($_POST['ticker'][$i])."',curr='".mysql_real_escape_string($_POST['curr'][$i])."',divcurr='".mysql_real_escape_string($_POST['divcurr'][$i])."',
	...
	$added=0;
		...
		$check=true;
				if(!empty($data))
				{
					...
					$query="INSERT into tbl_runnsecurities_replaced (status,name,isin,ticker,weight,curr,divcurr,sedol,cusip,countryname,sector,industry,subindustry,indxx_id,req_id) values ";
					$queryArray=array();
...
$this->Redirect("index.php?module=replacerunningsecurities&event=uploadSecuritiesforreplacement&indxx_id=".$_GET['indxx_id']."&req_id=".$_GET['req_id'],"Error in imput :".$errormsg,"error");	
	...
	elseif($added>=1)
		{
			$this->Redirect("index.php?module=replacerunningsecurities&event=addNewNext&total=".$added."&id=".$_GET['indxx_id']."&reqid=".$_GET['reqid'],"success");	
		}
				else
		{
...
{	   
	...
	function addNew()
	{
		...
		$this->Redirect("index.php?module=replacerunningsecurities&event=addstock","","success");
