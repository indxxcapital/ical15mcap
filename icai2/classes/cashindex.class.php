...
class Cashindex extends Application{
...
$this->addJs('js/flaty.js');
	...
	$this->_bodyTemplate="cashindex/index";
...
//	$this->addfield();
		...
		$userdata1=$this->db->getResult("select tbl_cash_index.* from tbl_cash_index where 1=1 ",true);
...
$headers .= 'From: Indexing <indexing@indxx.com>' . "\r\n"."CC: indexing@indxx.com". "\r\n";
		...
		if(mail($emailsids,"New cash isin  ",$msg,$headers))
		{
			...
			$this->Redirect("index.php?module=cashindextemp","Record added successfully!!!","success");	
		}
...
$headers .= 'From: Indexing <indexing@indxx.com>' . "\r\n"."CC: indexing@indxx.com". "\r\n";
		...
		if(mail($emailsids,"cash Index update ",$msg,$headers))
		{
			...
			$this->Redirect("index.php?module=cashindextemp","Record updated successfully!!!","success");	
		}
		
		
		
		$editdata=$this->db->getResult("select tbl_cash_index.* from tbl_cash_index  where tbl_cash_index.id='".$_GET['id']."'");
	...
	protected function delete(){
			...
			$strQuery = "delete from tbl_cash_index where tbl_cash_index.id='".$_GET['id']."'";
			$this->db->query($strQuery);
			$this->Redirect("index.php?module=cashindex","Record deleted successfully!!!","success");
	...
	function deleteindex()
	{
		...
		foreach($_POST as $key=>$val)
		{
			foreach($val as $key2=>$val2)
			{
				if(!empty($val2))
				{
					...
					$strQuery2 = "delete from tbl_cash_index where tbl_cash_index.id='".$val2."'";
		...
		$this->Redirect("index.php?module=cashindex","Records deleted successfully!!!","success");
		$this->show();	
	}
