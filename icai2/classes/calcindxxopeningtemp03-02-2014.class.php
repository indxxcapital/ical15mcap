...
elseif($type="open"){
	...
	///	exit;	
		...
		//$this->pr($ca_prices,true);
			...
			//echo $closeprices['ticker']."=>".$dividendPrice."<br>";
...
$this->db->query($shareUpdateQuery);	
			...
			if($oldname && $newname)
			{
			 ...
			 $nametickerUpdateQuery='UPDATE  tbl_indxx_ticker_temp  set name ="'.$newname.'" where indxx_id="'.$indxxKey.'"  and isin="'.$closeprices['isin'].'"'; 
...
echo "file Writ for ".$closeIndxx['code']."<br>";
	...
	*/
	...
	//	echo $newDivisor."<br>";
			...
			//exit;
				$insertQuery='INSERT into tbl_indxx_value_open_temp (indxx_id,market_value,indxx_value,date,olddivisor,newdivisor) values ("'.$closeIndxx['id'].'","'.$newmarketValue.'","'.$newindexvalue.'","'.$datevalue2.'","'.$oldDivisor.'","'.$newDivisor.'")';
		$this->db->query($insertQuery);	
