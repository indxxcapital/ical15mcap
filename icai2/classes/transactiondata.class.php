	...
	function index()
	{
		...
		foreach($getIndexes as $key=>$indexDetails)
		{
			...
			if($lastDayIndexValue!=0 && $lastDayInvestmentValue!=0)
			{
				...
				$tickersArray=$this->getNewTickersforIndex($indexDetails['id'],date);
				$totalMcap=0;
				
					if(!empty($tickersArray))
					{
						foreach($tickersArray as $isin2=>$ticker2)
						{
							$totalMcap=$totalMcap+$mCap_array[$isin2];
						}
				
				foreach($tickersArray as $isin=>$ticker)
				{
					...
					if($mCap_array[$isin]!="")
					$weight=number_format($mCap_array[$isin]/$totalMcap,15,'.','');
					else
					$weight=0;
					
					$ticker_inser_array[]="('".mysql_real_escape_string($ticker['name'])."','".mysql_real_escape_string($ticker['isin'])."','".mysql_real_escape_string($ticker['ticker'])."','".mysql_real_escape_string($weight)."','USD','USD','".mysql_real_escape_string($ticker['sedol'])."','".mysql_real_escape_string($ticker['cusip'])."','United States','".$indexDetails['id']."','1')";
...
function insetFinalPrice()
{
	...
	$query_share =$this->db->getResult("select * from tbl_weights_transaction where date='2014-12-31' and indxx_id ='4'") or die("err".mysql_error());
