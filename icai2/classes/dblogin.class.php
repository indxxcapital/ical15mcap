...
class Dblogin extends Application{
	...
	function index()
	{
		//echo "aa";
		//exit;
		$this->_baseTemplate="dblogin-template";
...
$this->addJs('assets/bootstrap/bootstrap.min.js');
	...
	if (!empty($_POST)){
		...
		//exit;
	$user=$this->db->getResult("select * from tbl_database_users where email='".$_POST['Username']."' ");
	...
	$this->setMessage("error",$this->l("Incorrect Password"));
		...
		//mysql_query("insert into tbl_db_user_login_time(status,type,user_id) values('1','1','".$_SESSION['User']['id']."')");
	...
	*/
		$lastlogintimequery=mysql_query("select * from tbl_db_user_login_time where user_id='".$_SESSION['User']['id']."' and type='1' order by dateAdded desc limit 0,1");
...
//echo "select * from tbl_employee where id='".$_SESSION['User']['id']."'";
	...
	function logout()
	{
		//mysql_query("insert into tbl_db_user_login_time(status,type,user_id) values('1','0','".$_SESSION['User']['id']."')");
		unset($_SESSION['User']);
