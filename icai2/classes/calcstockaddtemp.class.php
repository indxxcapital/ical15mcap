...
class Calcstockaddtemp extends Application{
...
//echo "select tbl_indxx_value.* from tbl_indxx_value where indxx_id='".$indxx['indxx_id']."' order by date desc ";
	...
	$newsecurities=	$this->db->getResult("select * from tbl_tempsecurities_replaced where req_id='".$indxx['id']."' and  indxx_id='".$indxx['indxx_id']."' ",true);
...
foreach($request_array as $id=>$indxx_array)			
{
	...
	if(!empty($indxx_array['newsecurity']) && !empty($indxx_array['index_value']))
	{
		...
		{
		 $share=	$indxx_array['index_value']['market_value']*$newsecurity['value']/$newsecurity['calcprice'];
		//echo "<br>";
		if($share){
			
			$insertTicker='Insert into tbl_indxx_ticker_temp set name="'.$newsecurity['name'].'", isin="'.$newsecurity['isin'].'", ticker="'.$newsecurity['ticker'].'", curr="'.$newsecurity['curr'].'", divcurr="'.$newsecurity['divcurr'].'",countryname="'.$newsecurity['countryname'].'",cusip="'.$newsecurity['cusip'].'",sedol="'.$newsecurity['sedol'].'",indxx_id="'.$id.'", status="1",weight="'.$security['value'].'"'; 
	...
	$this->db->query($insertPrice);
		...
		$insertshare='Insert into tbl_share_temp set date="'.$indxx_array['index_value']['date'].'", isin="'.$newsecurity['isin'].'", share="'.$share.'",indxx_id="'.$id.'"'; 
	
	$this->db->query($insertshare);	
	
	
	
	$insertweights='Insert into tbl_weights_temp set date="'.$indxx_array['index_value']['date'].'", isin="'.$newsecurity['isin'].'", share="'.$share.'",code="'.$indxx_array['index_value']['code'].'",price="'.$newsecurity['calcprice'].'",weight="'.$newsecurity['value'].'",indxx_id="'.$id.'"'; 
	
		$this->db->query($insertweights);
		
		$insertweightsopen='Insert into tbl_weights_open_temp set date="'.$indxx_array['index_value']['date'].'", isin="'.$newsecurity['isin'].'", share="'.$share.'",code="'.$indxx_array['index_value']['code'].'",price="'.$newsecurity['calcprice'].'",weight="'.$newsecurity['value'].'",indxx_id="'.$id.'"'; 
	...
	$this->db->query($insertweightsopen);	
	 ...
	 $query="SELECT  it.id, it.name, it.isin, it.ticker, 
							...
							left join tbl_share_temp sh on sh.isin=it.isin where it.indxx_id='" . $id . "' 
							 and sh.indxx_id='" .$id . "' and fp.indxx_id='".$id."' and fp.date='".$datevalue."'";

		$indxxprices=	$this->db->getResult($query,true);
	if(!empty($indxxprices))
	{
	$totalMarketCap=0;
	foreach($indxxprices as $securityprices){
		$totalMarketCap+=$securityprices['calcprice']*$securityprices['calcshare'];
		}
		//echo $totalMarketCap;
		//exit;
		 $newDivisor=$indxx_array['index_value']['newdivisor']-(($indxx_array['index_value']['market_value']-$totalMarketCap)/$indxx_array['index_value']['indxx_value']);
		
		$updateQuery='update tbl_indxx_value_temp set newdivisor="'.$newDivisor.'",olddivisor="'.$newDivisor.'" where  date="'.$indxx_array['index_value']['date'].'" and indxx_id="'.$id.'"';
		//exit;
