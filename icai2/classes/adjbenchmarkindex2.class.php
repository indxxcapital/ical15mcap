...
class Adjbenchmarkindex2 extends Application{
...
$this->addJs('js/flaty.js');
	...
	$this->_bodyTemplate="adjbenchmarkindex2/index";
...
//	$this->addfield();
		...
		$userdata1=$this->db->getResult("select tbl_adj_benchmark_index.* from tbl_adj_benchmark_index where 1=1 ",true);
	...
	{
			$this->_baseTemplate="inner-template2";
			$this->_bodyTemplate="adjbenchmarkindex2/add";
			$this->_title=$this->siteconfig->site_title;
			$this->_meta_description=$this->siteconfig->default_meta_description;
			$this->_meta_keywords=$this->siteconfig->default_meta_keyword;
		...
		{
		$this->db->query("INSERT into tbl_adj_benchmark_index set status='1',name='".mysql_real_escape_string($_POST['name'])."',curr='".mysql_real_escape_string($_POST['curr'])."',ticker='".mysql_real_escape_string($_POST['ticker'])."'");
...
$headers .= 'From: Indexing <indexing@indxx.com>' . "\r\n"."CC: indexing@indxx.com". "\r\n";
		...
		if(mail($emailsids,"New Benchmark Ticker  ",$msg,$headers))
		{
			...
			$this->Redirect("index.php?module=adjbenchmarkindex&event=addNew","Record added successfully!!!","success");	
		}
	...
	{	
	 ...
	 $this->validData[]=array("feild_label" =>"Currency Symbol",
		 							"feild_code" =>"curr",
								 "feild_type" =>"text",
								 "is_required" =>"1",
								 );	
	...
	function exportExcel()
	{
		...
		$excelres=mysql_query("select ticker,curr from tbl_adj_benchmark_index");
		$rowdata=array();
		while($exceldata=mysql_fetch_assoc($excelres))
		{
				//print_r($exceldata);
				$rowdata[]=array($exceldata['ticker']."||1|EQY_FUND_CRNCY|".$exceldata['curr']."|");
		}
	...
	protected function edit(){
		...
		$this->_baseTemplate="inner-template2";
			$this->_bodyTemplate="adjbenchmarkindex2/edit";
			$this->_title=$this->siteconfig->site_title;
			$this->_meta_description=$this->siteconfig->default_meta_description;
			$this->_meta_keywords=$this->siteconfig->default_meta_keyword;
		...
		$this->db->query("UPDATE tbl_adj_benchmark_index set name='".mysql_real_escape_string($_POST['name'])."',ticker='".mysql_real_escape_string($_POST['ticker'])."' where id='".$_GET['id']."'");
...
$headers .= 'From: Indexing <indexing@indxx.com>' . "\r\n"."CC: indexing@indxx.com". "\r\n";
		...
		if(mail($emailsids,"Benchmark Ticker update ",$msg,$headers))
		{
			...
			$this->Redirect("index.php?module=adjbenchmarkindex","Record updated successfully!!!","success");	
		}
		
		
		
		$editdata=$this->db->getResult("select tbl_benchmark_index.* from tbl_adj_benchmark_index  where tbl_adj_benchmark_index.id='".$_GET['id']."'");
	...
	protected function delete(){
		...
		$strQuery = "delete from tbl_adj_benchmark_index where tbl_adj_benchmark_index.id='".$_GET['id']."'";
		$this->db->query($strQuery);
			$this->Redirect("index.php?module=adjbenchmarkindex","Record deleted successfully!!!","success");
	...
	function deleteindex()
	{
		...
		foreach($_POST as $key=>$val)
		{
			foreach($val as $key2=>$val2)
			{
				if(!empty($val2))
				{
					...
					$strQuery2 = "delete from tbl_adj_benchmark_index where tbl_adj_benchmark_index.id='".$val2."'";
		...
		$this->Redirect("index.php?module=adjbenchmarkindex","Records deleted successfully!!!","success");
		$this->show();	
	}
